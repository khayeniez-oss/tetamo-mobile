...
function mapLikedProperty(row: PropertyRow, language: Language): LikedProperty {
  const title =
    language === "id"
...
export default function DashboardLikedScreen() {
  ...
  const ui = useMemo(() => {
    if (isId) {
      return {
        ...
        pageSubtitle: "Semua properti yang Anda sukai akan muncul di sini.",
  ...
  const loadLiked = useCallback(async () => {
    ...
    ((propertyRows || []) as PropertyRow[]).forEach((row) => {
      propertyMap.set(row.id, mapLikedProperty(row, language));
    });
      ...
      >
        <View style={styles.heroCard}>
          ...
          <View style={styles.totalPill}>
            <Heart color="#e6c15c" size={14} />
            <Text style={styles.totalPillText}>
              {ui.totalLiked}: {items.length}
            </Text>
          </View>
        </View>
                  ...
                  <View style={styles.badgeLayer}>
                    ...
                    {item.listingType === "disewa" && rentalTypeLabel ? (
                      <View style={[styles.badge, styles.rentBadge]}>
                        <Text style={styles.badgeText}>{rentalTypeLabel}</Text>
                      </View>
                    ) : null}
